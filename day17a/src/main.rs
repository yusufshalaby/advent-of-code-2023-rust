...
fn shortest_path(
    ...
    let mut dist: Vec<Vec<Vec<Vec<u32>>>> =
        vec![vec![vec![vec![u32::MAX; 4]; 4]; adj_list[0].len()]; adj_list.len()];
...
fn day17a(input: &str) -> Option<u32> {
    let map = input
        .lines()
        .map(|row| {
            row.chars()
    ...
    let mut dir_vecs: Vec<Vec<Vec<Vec<Vec<(usize, usize, usize, usize, u32)>>>>> =
        vec![vec![vec![vec![vec![]; 4]; 4]; map[0].len()]; map.len()];
    for i in 0..map.len() {
...
4322674655533"
    ...
    fn test_17a() {
        let input = input();
        assert_eq!(day17a(input), Some(102));
    }
}
